...
graphics_path <- '/rqaentr/fourth_proposal/src'
...
################################################################################
# (3) Reading data
file_ext <- "RQA3D_facelandmarks.dt"
...
#################################################################################
				### (4.2.4) Axis Selection
			      	...
			      	#axis <- c('zmuva_left_eye')
			      	axis <- c('zmuva_left_eye', 'zmuva_right_eye', 'zmuva_out_mouth',
...
for (eps_k in 1:length(EPS) ) {
	...
	A<-ATRk[, range:= (eps== recurrence_threshold ), by=. (Axis, dim, tau, eps)]
